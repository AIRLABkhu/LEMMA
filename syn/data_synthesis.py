...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 1000, batch_size):
        targets = targets_all[kk:min(kk+batch_size,1000)].to(args.device)

        data_type = torch.float
        inputs = torch.randn((targets.shape[0], 3, 224, 224), requires_grad=True, device=args.device,
                             dtype=data_type)
        ...
        for iteration in range(iterations_per_layer):
            ...
            # forward pass
            optimizer.zero_grad()
            outputs, _ = model_teacher(inputs_jit)
...
def parse_args():
    ...
    parser.add_argument("--device", default=0, type=int, help="enter cuda num")
...
def main_syn(ipc_id):
    ...
    model_teacher = imagenet_model_dict[args.arch_name](pretrained=True)
    ...
    model_verifier = imagenet_model_dict[args.arch_name](pretrained=True)
    model_verifier = model_verifier.to(device)
    model_verifier.eval()
