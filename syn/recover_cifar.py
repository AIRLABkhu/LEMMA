...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 100, batch_size):
        targets = targets_all[kk : min(kk + batch_size, 100)].to(args.device)

        data_type = torch.float
        inputs = torch.randn((targets.shape[0], 3, 32, 32), requires_grad=True, device=args.device, dtype=data_type)
...
def main_syn(args, ipc_id):
    ...
    device = args.device
    net, pretrain_model_path = cifar_model_dict[args.arch_name]
    model_teacher = net(num_classes=100)
