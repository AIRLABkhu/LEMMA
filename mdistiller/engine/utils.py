...
def adjust_ema_alpha(epoch, cfg):
    x_lower, x_upper = cfg.DISTILLER.EMA_FROM, cfg.SOLVER.EPOCHS
    y_upper, y_lower = cfg.DISTILLER.EMA
    gamma = cfg.DISTILLER.EMA_GAMMA
    return np.power(1 - np.power((epoch - x_lower) / (x_upper - x_lower), gamma), np.reciprocal(gamma)) * (y_upper - y_lower) + y_lower
