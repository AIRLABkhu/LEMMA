...
class CRD(Distiller):
    ...
    def __init__(self, student, teacher, cfg, num_data):
        super(CRD, self).__init__(student, teacher, cfg)
    ...
    def forward_train(self, image, target, index, contrastive_index, epoch, **kwargs):
        logits_student, feature_student = self.student(image)
        with torch.no_grad():
            _, feature_teacher = self.teacher(image, index)
            if self.update_teacher:
                self.teacher.update(index, epoch, logits_student, feature_student)
